        ...
        with torch.no_grad():
            for _ in range(steps):
                ...
                if len(temp_history) < self.window_size:
                    input_seq = [self.initial_state]*(self.window_size - len(temp_history)) + temp_history
                else:
                ...
                new_state[0] = pred.item() * 10  # 更新pH值
...
def main(args):
    ...
    for name, value, (low, high) in zip(
        ["溶氧(%)", "温度(℃)", "搅拌(rpm)", "O2流量(L/h)", "空气流量(L/h)", 
         "补料泵1(mL/h)", "补料泵2(mL/h)", "补料泵3(mL/h)", "补料泵4(mL/h)", "诱导剂浓度"],
        action[0],
        param_bounds
    ):
