...
def subtract_first_column(df):
    """对dm_o2、dm_air、dm_spump1、dm_spump2、dm_spump3、dm_spump4列，除第一列外，其余各列均减去第一列数值，此后，将第一列数据改为0.0001"""
    modified = False
    ...
    for sheet_name in df:
        ...
        for col in target_columns:
            if col in current_sheet.columns:
                first_col_values = current_sheet[col].iloc[0]
                # 除第一列外，其余各列均减去第一列数值
                current_sheet[col] = current_sheet[col] - first_col_values
                # 将第一列数据改为0.0001
                current_sheet[col].iloc[0] = 0.0001
                modified = True
        df[sheet_name] = current_sheet
    return df, modified
...
def process_excel_file(file_path):
    ...
    try:
        ...
        # 新增功能：对dm_o2、dm_air、dm_spump1、dm_spump2、dm_spump3、dm_spump4列进行处理
        df, sub_processed = subtract_first_column(df)
        ...
        if ts_processed or col_processed or sub_processed or zero_replaced or od_interpolated:
            # 备份原文件 
