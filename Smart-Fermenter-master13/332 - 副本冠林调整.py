...
class FermentationEnv(gym.Env):
    ...
    def __init__(self, model: nn.Module, scaler: object, initial_state: np.ndarray):
        ...
        epsilon = 1e-6
        self.param_bounds = [
			(max(epsilon, initial_params[0] - 1), initial_params[0] + 1), #PH
            (max(epsilon, initial_params[1] - 10.0), initial_params[1] + 10.0),  # 溶氧
            (max(0.0, initial_params[2] - 1.0), initial_params[2] + 1.0),        # 温度
            (max(epsilon, initial_params[3] - 100.0), initial_params[3] + 100.0),# 搅拌
            (max(epsilon, initial_params[4] - 100.0), initial_params[4] + 100.0),# O2流量
            (max(epsilon, initial_params[5] - 100.0), initial_params[5] + 100.0),# 空气流量
            ...
            (max(epsilon, initial_params[9] - 5.0), initial_params[9] + 5.0),    # 补料泵4
...
class SafeDataLoader:
    ...
    def load_initial_state(self) -> np.ndarray:
        try:
            ...
            # 检查必要列存在
            required_columns = ['Timestamp', 'm_ph', 'm_ls_opt_do', 'm_temp', 'm_stirrer',
                               'dm_o2', 'dm_air', 'dm_spump1', 'dm_spump2', 'dm_spump3', 'dm_spump4', 'induction']
            ...
            elif df['Timestamp'].dtype == np.float64:
                pass  # 已经是分钟格式
            else:
                raise ValueError("时间戳列格式不支持")
            
            # 验证目标时间
            target_time = float(self.timestamp)
            if target_time < 0:
                raise ValueError("目标时间戳不能为负值")
...
def main(args):
    print("\n" + "="*40)
    print(" BioProcess Optimization System v8.1")
    print("="*40)
    ...
    print(f"12小时预测: {od_predictions[12*1]:.3f}")  # 假设每10分钟一个数据点
    print(f"24小时预测: {od_predictions[24*1]:.3f}")
