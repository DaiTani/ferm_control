...
def optimize_inputs(args):
    ...
    # 提取当前m_ph和初始参数
    current_m_ph, initial_params = extract_initial_inputs(args.dataset, args.Timestamp)
    
    # 参数边界设置
    epsilon = 1e-6
    varbound = np.array([
        [max(epsilon, initial_params[0]-10), initial_params[0]+10],       # 溶氧设定值
        [initial_params[1]-1+epsilon, initial_params[1]+1-epsilon],       # 温度
        [max(epsilon, initial_params[2]-100), initial_params[2]+100],     # 搅拌速率
        [max(epsilon, initial_params[3]-100), initial_params[3]+100],     # O2流量
        [max(epsilon, initial_params[4]-100), initial_params[4]+100],     # 空气流量
        [max(epsilon, initial_params[5]-1), initial_params[5]+1],         # 补料泵1
        [max(epsilon, initial_params[6]-1), initial_params[6]+1],         # 补料泵2
        [max(epsilon, initial_params[7]-1), initial_params[7]+1],         # 补料泵3
        [max(epsilon, initial_params[8]-5), initial_params[8]+5],         # 补料泵4
        [epsilon, 1-epsilon]                                              # 诱导剂浓度
    ], dtype=np.float32)
    ...
    try:
        # 重新加载数据获取时间索引
        data_path = os.path.join(args.dataset, "data.xlsx")
        raw_df = pd.read_excel(data_path)
        raw_df['Timestamp'] = pd.to_datetime(raw_df['Timestamp'], dayfirst=True, errors='coerce')
        
        # 找到目标时间戳索引
        target_time = pd.to_datetime(args.Timestamp, dayfirst=True)
        time_diff = (raw_df['Timestamp'] - target_time).abs()
        closest_idx = time_diff.idxmin()
        
        # 检查时间窗口有效性
        if closest_idx < 19:
            raise ValueError(f"时间窗口不足，需要至少20个时间步，当前索引为{closest_idx}")
        
        # 构造优化后的特征向量
        optimized_feature = np.array([
            current_m_ph,                  # 保持当前m_ph值
            best_params[0],                # m_ls_opt_do
            best_params[1],                # m_temp
            best_params[2],                # m_stirrer
            best_params[3],                # dm_o2差分
            best_params[4],                # dm_air差分
            best_params[5],                # dm_spump1差分
            best_params[6],                # dm_spump2差分
            best_params[7],                # dm_spump3差分
            best_params[8],                # dm_spump4差分
            best_params[9]                 # induction
        ], dtype=np.float32)
        
        # 处理特征差分（与训练时一致）
        cumulative_indices = [4,5,6,7,8,9]
        for idx in cumulative_indices:
            prev_value = raw_df.iloc[closest_idx-1, idx+1]  # +1跳过Timestamp列
            optimized_feature[idx] = optimized_feature[idx] - prev_value
        
        # 归一化处理
        scaled_feature = scaler.transform([optimized_feature])
        ...
        # 替换最后一个时间步
        window = scaled_features[closest_idx-19:closest_idx+1]
        window[-1] = scaled_feature[0]
        ...
        with torch.no_grad():
            ...
            od_pred = prediction.item()
        
        print("\n" + "="*40)
        print(f" 优化后的OD600预测值: {od_pred:.4f}")
        print("="*40)
        
    except Exception as e:
        print(f"[错误] 预测过程中发生异常: {str(e)}")
        exit(1)
