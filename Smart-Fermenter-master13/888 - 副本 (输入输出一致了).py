...
def main(args):
    ...
    # 新增功能：输出数据到transform.xlsx
    original_data = dataset.copy()  # 原始数据
    normalized_data = pd.DataFrame(data_loader.scaler.transform(dataset.iloc[:, 1:12]), columns=dataset.columns[1:12])  # 归一化后的数据
    denormalized_data = pd.DataFrame(data_loader.scaler.inverse_transform(normalized_data), columns=dataset.columns[1:12])  # 逆归一化后的数据
