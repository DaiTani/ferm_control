...
def objective_function(X, model, scaler, dataset_path):
    if not hasattr(objective_function, 'test_loader'):
        try:
            ...
            ]].values  # 改为values获取numpy数组
            
            # 处理累积量特征
            cumulative_indices = [4,5,6,7,8,9]  # dm_o2到dm_spump4的索引
            for idx in cumulative_indices:
                features[:, idx] = np.diff(features[:, idx], prepend=0)
    ...
    avg_rmse = total_loss / len(objective_function.test_loader)
    return -avg_rmse

# ---------------------- 优化主函数 ----------------------
def optimize_inputs(args):
    print("\n" + "="*40)
    print(" 智能发酵参数优化系统 v1.1")
    print("="*40)
    ...
    # 参数边界
    varbound = np.array([
        [max(0.1, initial_params[0]-10), initial_params[0]+10],  # 防止初始值为0
        [initial_params[1]-1, initial_params[1]+1],
        [max(0.1, initial_params[2]-100), initial_params[2]+100],
        [max(0.1, initial_params[3]-100), initial_params[3]+100],
        [max(0.1, initial_params[4]-100), initial_params[4]+100],
        [max(0.1, initial_params[5]-1), initial_params[5]+1],
        [max(0.1, initial_params[6]-1), initial_params[6]+1],
        [max(0.1, initial_params[7]-1), initial_params[7]+1],
        [max(0.1, initial_params[8]-5), initial_params[8]+5],
        [0, 1]
    ], dtype=np.float32)
    ...
    for name, opt_val, init_val in param_info:
        ...
        print(f" {name:<15} 初始值: {init_val:.2f} → 优化值: {opt_val:.2f} ({change_str})")
