...
class RealData(Dataset):
    def __init__(
        ...
        for x, y in zip(X, Y):
            ...
            df[self.y_var[0]] = y
            all_data.append(df)
        self.dataset_df = pd.concat(all_data, ignore_index=True)

        # 如果指定了时间戳，筛选对应数据
        if self.timestamp is not None:
            self.dataset_df['Timestamp'] = np.arange(len(self.dataset_df))
            self.dataset_df = self.dataset_df[self.dataset_df['Timestamp'] <= self.timestamp]
