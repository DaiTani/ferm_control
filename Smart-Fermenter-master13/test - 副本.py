...
def main():
    ...
    parser.add_argument("--pred_steps", default=1, type=int, help="Number of prediction steps")  # 新增参数
    ...
    data_path = os.path.join(args.dataset, "28", "data.xlsx")  # 根据实际路径调整
    ...
    rmse_array = np.full_like(first_column, rmse)  # 创建一个与 first_column 形状相同的数组，填充 rmse 值
    refy_array = np.full_like(first_column, refy)  # 创建一个与 first_column 形状相同的数组，填充 refy 值

    # 将第一列数据添加到 results.npz 的最左侧
    timestamp_data = np.column_stack((first_column, preds, labels, rmse_array, refy_array))

    # 保存到 results.npz
    np.savez(
        weights.split("/weights")[0] + "/results.npz",
        Timestamp=timestamp_data,  # 将 combined_data 改为 Timestamp
        preds=preds,
        labels=labels,
        rmse=rmse,
        refy=refy,
    ...
    plt.plot(labels, label="True Values", color="black", linewidth=2)
    for step in range(args.pred_steps):
        plt.plot(preds[:, step], label=f"Predicted Step {step + 1}")
    plt.legend()
