...
class SafeDataLoader:
    ...
    def load_initial_state(self) -> Tuple[np.ndarray, float]:
        try:
            ...
            print(df.head(11)) 
            # 数据清洗：使用均值填补缺失值 
            features = np.where(np.isnan(features),  np.nanmean(features),  features)
            features = np.where(np.isinf(features),  np.nanmean(features),  features)
            
            features[5:11] = np.where(features[5:11]  <= 0, np.nanmean(features[5:11]),  features[5:11])
            
            scaled_features = self.scaler.transform([features]) 
            timestamp = df.iloc[closest_idx]['Timestamp'] 
            print("\n[原始数据加载完成]")
            print("原始数据前5行：")
            print(df.head(11))
            return scaled_features[0], timestamp 
...
def main(args):
    ...
    formatted_time = format_timestamp(initial_timestamp)
    
    print("\n{:<15} {:<15} {:<15} {:<10} {:<15}".format("参数", "原始值", "优化值", "变化率", "时间"))
    print("-" * 85)
    for i in range(11):
        ...
        print(f"{param_names[i]:<15} {orig:<15.2f} {optim:<15.2f} {change_str:<10} {formatted_time:<15}")
    ...
    six_hours_later = current_time + 6 * 60 
    twenty_four_hours_later = current_time + 24 * 60 
    peak_index = od_predictions.index(max(od_predictions)) 
    peak_time = current_time + (peak_index // 60) * 60 
 
    print("\nOD600预测：")
    print(f"当前值: {od_predictions[0]:.3f} (时间: {format_timestamp(current_time)})")
    print(f"6小时后: {od_predictions[6*6]:.3f} (时间: {format_timestamp(six_hours_later)})")  
    print(f"24小时后: {od_predictions[24*6]:.3f} (时间: {format_timestamp(twenty_four_hours_later)})")
    print(f"峰值浓度: {max(od_predictions):.3f} (时间: {format_timestamp(peak_time)})")
