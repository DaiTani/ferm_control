...
def cumulative2snapshot(data: np.ndarray, cumulative_cols: List[int]) -> np.ndarray:
    """将累积变量转换为瞬时值"""
    processed = data.copy()
    for col in cumulative_cols:
        tmp = np.insert(processed[:, col], 0, processed[0, col])[:-1]
        processed[:, col] = processed[:, col] - tmp
    return processed
...
class FermentationEnv(gym.Env):
    ...
    def __init__(self, model: nn.Module, scaler: object, dataset: pd.DataFrame, timestamp: float, y_mean: float, y_std: float):
        ...
        # 参数的优化范围参考
        param_bounds = [
            (max(epsilon, initial_params[0] - 2.0), initial_params[0] + 2.0),  # 空气流量
            (max(epsilon, initial_params[1] - 2.0), initial_params[1] + 2.0),  # 溶氧
            (max(3.0, initial_params[2] - 1.0), min(8.0, initial_params[2] + 1.0)),  # PH
            (max(epsilon, initial_params[3] - 20.0), initial_params[3] + 20.0),  # 搅拌
            (max(0.0, initial_params[4] - 2.0), initial_params[4] + 2.0),  # 温度
            (max(epsilon, initial_params[5] - 2.0), initial_params[5] + 2.0),  # O2流量
            (max(epsilon, initial_params[6] - 2.0), initial_params[6] + 2.0),  # 补料泵1
            (max(epsilon, initial_params[7] - 2.0), initial_params[7] + 2.0),  # 补料泵2
            (max(epsilon, initial_params[8] - 2.0), initial_params[8] + 2.0),  # 补料泵3
            (max(epsilon, initial_params[9] - 1.0), initial_params[9] + 1.0),  # 补料泵4
...
class SafeDataLoader:
    def __init__(self, dataset_path: str, timestamp: str, scaler_path: str):
        ...
        self.cumulative_cols = [
            "dm_o2", "dm_air", "dm_spump1", "dm_spump2", "dm_spump3", "dm_spump4"
        ]  # 根据实际变量名调整索引
        self.x_columns = [
            "m_ph", "m_ls_opt_do", "m_temp", "m_stirrer", "dm_o2", "dm_air",
            "dm_spump1", "dm_spump2", "dm_spump3", "dm_spump4", "induction"
        ]
        self.cumulative_idx = [self.x_columns.index(col) for col in self.cumulative_cols]
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        # 应用cumulative2snapshot预处理
        raw_data = df.iloc[:, 1:12].values  # 提取特征列
        processed_data = cumulative2snapshot(raw_data, self.cumulative_idx)
        df.iloc[:, 1:12] = processed_data
...
def main(args):
    ...
    initial_params = initial_obs[:11].copy()
    ...
    # 输出数据到transform.xlsx（修复特征名称警告）
    original_data = dataset.copy()
    normalized_values = data_loader.scaler.transform(dataset.iloc[:, 1:12].values)
    normalized_data = pd.DataFrame(normalized_values, columns=dataset.columns[1:12])
    denormalized_values = data_loader.scaler.inverse_transform(normalized_values)
    denormalized_data = pd.DataFrame(denormalized_values, columns=dataset.columns[1:12])
