...
def objective_function(X, model, scaler):
    ...
    if X.shape[1] != 11:  # 检查特征数量
        raise ValueError(f"Expected 11 features, but got {X.shape[1]} features.")
...
# 遗传算法优化
def optimize_inputs(model_path, scaler_path, input_dim, hidden_dim, output_dim, n_layers):
    # 加载模型和标准化器
    ...
    print(f"Additional Feature: {best_solution[10]}")  # 第 11 个特征
...
if __name__ == "__main__":
    ...
    # 输入特征的维度
    input_dim = 11  # 输入特征的数量
    output_dim = 1  # 输出维度

    # 运行遗传算法优化
    optimized_inputs, best_value = optimize_inputs(model_path, scaler_path, input_dim, args.hidden_dim, output_dim, args.num_layers)
